terraform {
  backend "s3" {                     # backend means where it's gonna store the tfstate file. so here it's s3 bucket. 
    ...
    key = "week16-terraform-final-project"         #it is just a name of the file   # key is just subfolders  /terraform will create folder called tfstate in the terraform-state-file-d bucket  or it is just a name of the file
